...
### 部署前准备
在开始部署之前，需要在 Github developer 设置页面完成以下准备工作：

1. **创建 Github OAuth App**
   - 登录 [Github developer 设置页面](https://github.com/settings/developers)
   - 点击 左侧 “OAuth Apps” 的标签
   - 点击 “New OAuth App” 新增OAuth App
   - 填写应用名称和回调地址
   - `Homepage URL` 内填入你需要部署至的域名 例如 `https://moemail.app`
   - `Authorization callback URL` 内填入你的域名+回调地址，例如 `https://moemail.app/api/auth/callback`
   - 创建完成后 点击 “Generate a new client secret” 
     - 生成 `Client Secret` 即 `AUTH_GITHUB_SECRET`
     - 并记录下 `Client ID` 即 `AUTH_GITHUB_ID`，后续配置需要用到

至此，我们获得了 `AUTH_GITHUB_SECRET` , `AUTH_GITHUB_ID`

在开始部署之前，需要在 Cloudflare 控制台 完成以下准备工作：

1. **创建 D1 数据库**
    - 登录 [Cloudflare 控制台](https://dash.cloudflare.com/)
    - 选择 “存储与数据库” -> “D1 SQL 数据库”
    - 创建一个数据库（例如：moemail）
    - 记录下数据库名称`DATABASE_NAME`和数据库 ID`DATABASE_ID`，后续配置需要用到

2. **创建 KV 命名空间**
    - 登录 [Cloudflare 控制台](https://dash.cloudflare.com/)
    - 选择 “存储与数据库” -> “KV”
    - 创建一个 KV 命名空间（例如：moemail）
    - 记录下命名空间 ID`KV_NAMESPACE_ID`，后续配置需要用到
...
3. **创建 Pages 项目**
    ...
    - 输入项目名称后，点击 “创建项目” 即可，不需要上传任何文件以及点击“部署站点”，之后我们会通过 本地运行Wrangler 或者通过
      Github Actions 自动部署
    - 返回 “Workers 和 Pages” , 记录下右边的账户ID `CLOUDFLARE_ACCOUNT_ID`

4. **为 Pages 项目添加 AUTH 认证相关的 SECRETS**
    - 在 Overview 中选择刚刚创建的 Pages 项目
    - 在 Settings 中选择变量和机密
    - 添加 上一步获取到的 `AUTH_GITHUB_SECRET` , `AUTH_GITHUB_ID` , `AUTH_SECRET` [自定义随机字符串]

至此，我们获得了 `DATABASE_NAME` , `DATABASE_ID` ,`KV_NAMESPACE_ID` ，`CLOUDFLARE_ACCOUNT_ID`
...
[moemail-auto-deploy](https://github.com/Ktovoz/moemail-auto-deploy)项目提供了自动化部署方案

使用此项目方案可以仅需手动登录即可自动创建并获取以下参数
